...
def SimuladorImpresora(tiempo_simulacion : int,tiempo_llegada : int,cola:ColaEncadenada):
    ...
    while reloj < tiempo_simulacion:
        ...
        if impresora == 0 and not cola.vacia():
            ...
            impresora = 30 if hojas_trabajo >= 30 else hojas_trabajo
        if impresora > 0:
            hojas_trabajo = cola.recuperar().getHojas()
            if impresora == 1 :
                if hojas_trabajo > 1: 
                    cola.recuperar().setHojas(hojas_trabajo - 1)
    ...
    print(f"El tiempo promedio de impresion fue de {(tiempo_total_atencion / trabajos_impresos)/60:.2f} minutos")
SimuladorImpresora(3600,0.02,cola = ColaEncadenada())
